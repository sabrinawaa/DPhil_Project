...
def main(args):
    ...
    #mm
    try:
        setup = partrec_gaussian_optimiser_utils(file_directory=dir,input_filename=f'topas_main_{run_no}.txt',topas_dir ='/home/sabrinawang/Applications/')
        ...
        for p in (
            dir + f"topas_main_{run_no}.txt",
            dir + f"CLEAR_line_{run_no}.header",
            dir + f"CLEAR_line_{run_no}.phsp",
        ):
            try:
                os.remove(p)
